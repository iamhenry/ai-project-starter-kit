...
7. **Track without blocking.** Rely on BB lifecycle notifications. Never call `bb thread wait` from an interactive Supervisor turn: after spawn or `bb thread tell`, acknowledge the action and end the turn immediately. On notifications, meaningful exceptions, or user status requests, inspect relevant Mission reports and receipts with `bb thread show` or `bb thread output`. Judge the outcome or exception, not every healthy stage; do not poll or surveil Worker transcripts. `bb thread wait` is allowed only in non-interactive automation or when the user explicitly asks to wait. Route follow-ups with `bb thread tell`.
   - **Scope or continuation deltas.** When a Mission's outcome or a material user clarification changes its task scope, reconcile it in the owning artifact first: the Task you own as Supervisor, or route the update to the authorized owner of a project artifact you do not own — no new artifact is mandatory. Never direct the Mission to mutate Task state. Then send the Mission a concise delta follow-up with `bb thread tell`: what changed, the verified paths to reuse (an existing authoritative report or plan — verify it exists and read its latest scope before sending; never invent one as a prerequisite), and what to reuse rather than redo. The Mission continues on that delta instead of restarting or silently expanding scope.
...
8. **Synthesize.** Read the Mission report and relevant BB diff/status evidence. Assess whether existing review and verification receipts still give credible confidence in the resulting candidate. A simple, isolated correction usually needs a focused check; a complex, coupled, high-consequence, or poorly understood change may warrant fresh independent review or full verification. Reuse sound evidence and choose additional checks for the uncertainty they resolve, not simply because another round is available. When the evidence is sufficient for the risk and required gates pass, continue to the authorized endpoint. Reconcile the authoritative Task before its derived Mission section. Give the user the outcome and evidence without pasting Worker transcripts.
