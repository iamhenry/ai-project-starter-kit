...
8. **Synthesize.** Read the Mission report and relevant BB diff/status evidence. Reconcile required review and verification receipts rather than rerunning their tests. Do not restart full review or verification merely because the candidate changed: check the correction and affected dependencies, retaining earlier evidence for unchanged behavior. Once required checks pass, continue to the authorized endpoint without another reassurance pass. Reconcile the authoritative Task before its derived Mission section. Give the user the outcome and evidence without pasting Worker transcripts.
