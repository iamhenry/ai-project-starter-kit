...
- `source_identity`: the frozen source candidate under review, forwarded
  unchanged: `candidate_id`, immutable `candidate_ref`, `candidate_digest`,
  `base_head` as provenance, and `declared_paths`. Build/runtime identity is
  intentionally attached later for Accept. A branch name, path, timestamp, or
  `current` checkout alone is not identity.
...
Missing `isa_path`, `locked_slice`, any required source identity field, or
...
- `REVISE_CODE`: route the findings to the implementation owner. After
  revision, rerun this gate with a newly frozen source identity and the
  applicable locked slice; never silently reuse an identity for changed code.
- `ASK_USER`: route missing, conflicting, or ambiguous ISA inputs to the caller.
- `APPROVE_CODE`: route the same locked slice and frozen source identity to
  integration and `verification-gate`. If integration changes reviewed content
  or a relevant dependency assumption, rerun this gate only for affected lanes.

## Output

Use the existing `## Code Quality Gate Result` structure from `SKILL.md`. Keep
its decision, score, findings, required changes, and next-action fields
unchanged. In `Reviewed`, name `isa_path`, `locked_slice`, every field of
`source_identity`, the diff/changed files, summary, and supplied checks instead
of `plan.md`/`issue.md`. Forward the source identity unchanged; do not require
or invent build/runtime fields and do not add leaf verdicts.
