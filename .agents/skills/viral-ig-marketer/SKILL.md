...
| App codebase            | GitHub repo (`app.githubRepo` in config.json) read via the `gh` CLI → cached in `references/app-brief.md`                                                     | Unlimited (read-only)  | Read-only, no forks or PRs                | Use cached app-brief.md if repo unavailable                                                                                                                           |
...
6. **App intelligence check** — only run if current quadrant is "High views + flat MRR". Skip entirely otherwise.
   - Read the GitHub repo with the `gh` CLI in read-only mode (use cached `references/app-brief.md` if recently read; refresh monthly or when codebase changes)
