...
- Create-issue structure: 40
  ...
  - 10 implementation checklist uses action prefixes and files or locations; phases only when useful.
   - 10 verification target includes `Platform` (legacy `Verification Mode`), `Objective`, `Falsifier`, `Primary Flow`, `Regression Check`, `Mechanical`, `Observable`, `Pass Criteria`, and `Blocked Conditions`. Mechanical covers relevant mechanical health or a machine-checkable condition; together the declared lanes must distinguish the Objective from its Falsifier. Existing build/lint/typecheck plus independent actual-product evidence can suffice for simple UI work. Additional assertions should protect distinct changed failure modes, not satisfy a template. Observable must be a retained path, or `n/a` only for a genuinely internal `non-ui` change with no changed user or consumer-observable behavior.
