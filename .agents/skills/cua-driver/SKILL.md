...
and training data. Invoke when the user explicitly asks to record a session or
when `verification-gate` requires video because motion itself is under test;
the skill does not otherwise auto-enable this. CLI surface:
