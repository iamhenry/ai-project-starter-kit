...
`flow-add-step.command` is an MCP tool name. `args` is a JSON string, not an object. Omit `args` for a no-argument tool. (In this bundle there is no MCP client. This reference's recorder workflow describes upstream capability, but the recorder tools are unavailable in this repo: they always write under `<project_root>/.argent/flows/`, while flow YAML lives ONLY at `_ai/task/{SLUG}/reproduction/flows/<safe-name>.yaml` per the parent's flow-storage override — author the YAML directly with file tools instead, executing steps live as described.)
