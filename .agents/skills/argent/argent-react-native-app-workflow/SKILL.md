...
**Before running commands**, read the project's build and run configuration yourself (in this bundle there is no `argent-environment-inspector` subagent — use the manual fallback below).
...
Read the project's `package.json` scripts and test config for test commands. For interactive UI testing with automatic screenshot verification, use the `argent-test-ui-flow` skill.
