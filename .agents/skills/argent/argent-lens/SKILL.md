...
> **Prerequisite — feature flag.** This workflow is gated behind the `argent-lens` flag (off by default). Run `argent enable argent-lens` once before using it. (Enabling feature config is a user decision — ask the user first in this bundle.) If `propose_variant` / `await_user_selection` come back not-found, the flag is off — enable it and retry.
