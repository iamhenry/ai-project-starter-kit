...
If you delegate simulator tasks to sub-agents, make sure they have MCP permissions. (In this bundle there is no MCP: call the tools through the parent skill's CLI translation — `argent run <tool> --args '<json>'`.)
...
**Check tool flags before first use.** (Upstream deferred-schema ToolSearch does not apply here.) Gesture tools (`gesture-tap`, `gesture-swipe`, `gesture-pinch`, `gesture-rotate`, `gesture-custom`) have numeric parameters — pass them as JSON numbers, never strings, or validation will coerce and reject them. Get each tool's exact flags with `argent tools describe <tool>` before calling it.

## 2. Best Practices

1. **Before tapping, re-verify the target from a fresh `describe`** (this replaces upstream's `tapping_rule` from an `argent.md` rules file, which is not bundled).
...
Use full-resolution screenshots only when saving baseline/current PNG files for comparison. In that case, suppress the image block so the full-size PNG is not loaded into agent context (through the CLI this means the tool prints the PNG's file path instead of attaching pixels):
