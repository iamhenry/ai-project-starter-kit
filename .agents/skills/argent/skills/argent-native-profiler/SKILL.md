...
**Tip:** For reproducible before/after comparisons, record the interaction sequence as a flow using the `argent-create-flow` skill before the first profiling run. Replay with `argent flow run <path.yaml>` on subsequent runs to eliminate interaction variance.
