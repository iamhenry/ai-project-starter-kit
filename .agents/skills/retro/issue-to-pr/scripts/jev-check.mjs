...
const cfg = JSON.parse(read("references/jev-questions.json"));
...
const definitionHash = hash(["../SKILL.md", "references/issue-to-pr.md", "references/jev-questions.json", "scripts/jev-check.mjs"].map(read).join("\n"));
...
for (const s of sources) {
  ...
  if (!read(file).replace(/\s+/g, " ").includes(text)) throw new Error(`Stale question: "${text}" no longer appears in ${file}. Update references/jev-questions.json.`);
}
...
const decisions = read("references/issue-to-pr.md").split("\n").filter((l) => /^\| (Confirmed|Observed)/.test(l))
...
for (const f of [join(SKILL, ".env"), join(SKILL, "../.env")]) if (!process.env.OPENROUTER_API_KEY && existsSync(f)) process.loadEnvFile(f);
