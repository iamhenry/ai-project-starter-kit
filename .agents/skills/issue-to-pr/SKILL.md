...
When resolving referenced skills, agents, or commands, check the project's locations and OpenCode's configured global directory (commonly `~/.config/opencode`). Projects may rely entirely on global resources rather than importing local copies. Use the available configuration to locate the same owner, pass its resolved path onward, and distinguish a failed lookup from a genuinely unavailable resource before declaring a prerequisite missing.
