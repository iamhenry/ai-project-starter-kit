...
| Verification | Use the smallest check that can detect a meaningful failure. Low-risk mechanical or documentation changes need a focused diff and relevant file/content check, not separate review and verification agents. Require independent review for substantive behavior, security, public-contract, or migration changes. | After corrections, rerun only checks whose conclusions could be invalidated; reuse unaffected evidence. Broaden verification only for a concrete dependency, failure, or unresolved risk, and name it first. Use `verification-gate` when verification is fuzzy or high-impact; iOS/macOS via xcodebuildmcp. |
